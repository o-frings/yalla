...
The model's behaviour is clearest as projected muscle gain under different training regimes. A seeded, reproducible simulation (\texttt{research/simulate.mjs}) drives the Monte Carlo forecast below forward for each regime, in the app's colours; every number regenerates from the same seed.
...
\addplot[appAccent, line width=1pt] table[x=week,y=progMid]{research/sim-regimes.dat}; \addlegendentry{plan, progressing}
\addplot[appBlue, line width=1pt] table[x=week,y=flatMid]{research/sim-regimes.dat}; \addlegendentry{plan, no overload}
\addplot[appViolet, line width=1pt] table[x=week,y=irrMid]{research/sim-regimes.dat}; \addlegendentry{irregular ($\sim$55\%)}
\addplot[appRed, line width=1pt] table[x=week,y=lossMid]{research/sim-regimes.dat}; \addlegendentry{below maintenance}
\end{axis}\end{tikzpicture}
\caption{Projected muscle gain over 24 weeks under four regimes at a novice base rate (median lines; shaded 10--90\% bands on the best and worst). Same volume drives the progressing and no-overload lines, so their gap is progressive overload; below maintenance the median crosses zero (net loss). Bands are chiefly inter-individual variability~\cite{hubal}.}\label{fig:regimes}
\end{figure}

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, xmax=24, ymin=0, ylabel={projected muscle gain (\%)}, legend pos=north west, legend cell align=left]
...
\addplot[appAccent] table[x=week,y=pplMid]{research/sim-plangain.dat}; \addlegendentry{Push/Pull/Legs 6x}
\end{axis}\end{tikzpicture}
\caption{Projected muscle gain for three standard plans, each executed with progressive overload (median, 10--90\% bands). More weekly volume per muscle projects more gain, with diminishing returns and heavily overlapping bands.}\label{fig:plangain}
\end{figure}

The starting point sets the scale. Figure~\ref{fig:start} runs the same adequate, progressing plan from three training-age base rates~\cite{damas}: a novice gains most and fastest, an advanced lifter least, and the bands overlap throughout. The app reads this base rate from the lifter's own logged history.
...
\end{axis}\end{tikzpicture}
\caption{The same adequate, progressing plan from three starting points (training-age base rates): projected muscle gain, median and 10--90\% bands (novice and advanced). Newer lifters gain faster; all bands are wide and overlap.}\label{fig:start}
\end{figure}
...
\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[xbar, width=\linewidth, height=5cm, xmin=0, xmax=7,
  bar width=10pt, enlarge y limits={abs=0.7}, y=0.72cm,
  ...
  tick label style={font=\footnotesize}, axis lines=left, ytick align=outside,
  grid=major, grid style={black!8},
  ytick={0,1,2,3,4}, yticklabels={age, volume, effort, progression, experience}]
\addplot[fill=appAccent!30, draw=appAccent] table[x=swing, y=y] {research/sim-sens.dat};
\end{axis}\end{tikzpicture}
\caption{Parameter sensitivity of the Monte Carlo forecast, sorted by influence: how far the median 16-week gain swings as each input moves across a plausible range, around a novice plan baseline. Training experience and progression dominate; age moves it least.}\label{fig:sens}
\end{figure}
