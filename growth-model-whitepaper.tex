...
Most training apps record what was lifted; few say what it means for muscle. Yalla estimates, per muscle and for the body overall, whether recent training is enough to grow, maintain, or slowly lose size, using only the sets, loads, and reps a user logs. This document specifies that model: the quantities it computes, the equations that combine them, the decision rules that produce a per-muscle verdict, and the evidence behind each parameter. The goal is a feedback signal a user or coach can audit rather than a black box, and one whose parameters trace to the resistance-training literature. Sections~\ref{sec:overview}--\ref{sec:plan} define the model, Section~\ref{sec:basis} lists the evidence with a grade for each source, and Section~\ref{sec:verification} describes the checks that accompany the model in the absence of a body-composition study.
...
Overload via added load \emph{or} reps, so the trend tracks both, not tonnage & Plotkin et~al.~\cite{plotkin} (RCT; load- and rep-progression gave near-identical growth); mechanism in Schoenfeld~\cite{sch10}. & Moderate\\
