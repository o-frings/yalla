...
The figures that follow project muscle gain under different training regimes from the Monte Carlo forecast defined next. A seeded script (\texttt{research/simulate.mjs}) generates them and reproduces every value on re-run.
...
\end{equation}
where $\rho(D)$ is the dose--response of \eqref{eq:doseresp}~\cite{sch17,drr}, $\eta$ scales with proximity to failure~\cite{failureprox}, $o$ is near 1 while progressing and drops without added load~\cite{plotkin}, $b$ is a training-age base rate~\cite{damas}, $\alpha$ is an age factor that attenuates hypertrophy past about 30~\cite{peterson}, and $m$ is the inter-individual response multiplier, drawn lognormal to span the range Hubal et~al.~\cite{hubal} observed (cross-sectional-area change from about $-2\%$ to $+59\%$). Sex is a separate lever set near neutral: relative hypertrophy is comparable between sexes~\cite{roberts}, so it bears on absolute mass while the percentage gain here stays about the same for either. Below maintenance the increment turns negative (atrophy~\cite{mujika}), so a starved scenario trends down and the axis extends below zero. Most terms are well constrained by the literature; $b$ carries a wide prior, and $m$ dominates the spread, so the width of the band is the honest output.

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, xmax=24, ymin=-6, ymax=25, ylabel={projected muscle gain (\%)}, legend pos=north west, legend cell align=left]
...
\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, xmax=24, ymin=0, ymax=25, ylabel={projected muscle gain (\%)}, legend pos=north west, legend cell align=left]
...
On the same adequate, progressing plan, a novice gains most and an advanced lifter least across three training-age base rates~\cite{damas}, with bands that overlap throughout (Fig.~\ref{fig:start}).

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, xmax=24, ymin=-1, ymax=25, ylabel={projected muscle gain (\%)}, legend pos=north west, legend cell align=left]
...
Across a plausible range for each input, training experience and progression move the 16-week median gain most and age least (Fig.~\ref{fig:sens}); Section~\ref{sec:verification} gives the matching sensitivity of the state thresholds.
