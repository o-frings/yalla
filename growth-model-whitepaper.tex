...
Set strength is summarised by the Epley one-rep-max estimate, with a rep denominator $K$ (population default $K=30$; Section~\ref{sec:kcal} calibrates it per lifter):
...
\end{tabular}
\end{center}
A single set of landmarks is applied to every muscle. Real per-muscle landmarks differ, but one baseline keeps the output legible. The maintenance floor is deliberately low: built muscle is held on very little (as little as roughly one-ninth of the building volume in young trained adults), so a 2-set floor avoids flagging loss too early.

\subsection{Continuous backbone}
The four landmarks discretise a continuous relationship. We model the fraction of the attainable per-muscle growth stimulus at $s$ effective sets per week as a saturating curve $\rho(s)$, calibrated so the practical target captures about $80\%$ of what is attainable:
...
\midrule
Graded volume dose--response; ${\sim}10$ sets as a practical target & Schoenfeld, Ogborn \& Krieger 2017, \emph{J Sports Sci}: each added weekly set ${\approx}\,{+}0.37\%$ muscle, benefit rising across the studied range. & High\\
Fractional (0.5) counting of indirect sets; diminishing returns; frequency neutral at equal volume & Pelland et~al.\ 2026, \emph{Sports Medicine} (peer-reviewed; 67 studies, 2{,}058 subjects). doi:10.1007/s40279-025-02344-w & High\\
Continuous backbone $\rho(s)$ and landmark derivation & Derived from the dose--response curves above (Schoenfeld 2017; Pelland 2026). & High\\
Effort factor $\eps$; hypertrophy comparable across ${\sim}0$--4 RIR, dropping off when easier & Robinson/Pelland 2024 proximity-to-failure meta-regression; Refalo et~al.\ 2024 RCT (failure vs.\ 1--2 RIR). & High\\
$\MEV/\MAV$ landmark structure (${\sim}10$ to 20 sets) & Israetel et~al.\ (Renaissance Periodization) volume-landmarks framework. & Low\\
Maintenance floor $\MV$; size held on ${\sim}1/3$ (older) to ${\sim}1/9$ (young) of building volume & Bickel, Cross \& Bamman 2011, \emph{Med Sci Sports Exerc} (RCT, 16-wk build plus 32-wk reduced dose). & Moderate\\
Decline below maintenance stimulus; ${\sim}2$--4-week strength retention, then loss & Mujika \& Padilla 2000 detraining review; cross-sectional-area loss from ${\sim}2$--3 weeks of cessation. & Moderate\\
Overload via added load \emph{or} reps, so the trend tracks both, not tonnage & Plotkin et~al.\ 2022, \emph{PeerJ} (RCT; load- and rep-progression gave near-identical growth); mechanism in Schoenfeld 2010. & Moderate\\
Epley one-rep-max $\varphi$; self-calibrated denominator $K$ & Standard load--rep relationship; $K$ fitted from the lifter's own Max sets \eqref{eq:kcal}. & Method\\
\bottomrule
\end{tabular}
\end{center}

\section{Verification}\label{sec:verification}
The model is not validated against body-composition measurement (see the limitations that follow), but three reproducible checks accompany it (the \texttt{research/} folder in the repository), each raising rigor without new measurement.
...
\begin{itemize}[leftmargin=*]
\item \textbf{Validation status.} This is a mechanistic model grounded in the literature and internally consistent, but it has not been validated against direct body-composition measurements (DXA, ultrasound, or MRI). Its thresholds are derived from published dose--response findings, not fitted to an outcome dataset, and no predictive accuracy interval is claimed. The checks in Section~\ref{sec:verification} test internal consistency and coherence, not external accuracy.
...
\item \textbf{Conservative by design.} The maintenance floor and the ``at risk'' threshold are both set so the model under-reports muscle loss rather than over-reports it.
\end{itemize}
