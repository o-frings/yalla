...
Every figure here is a Monte Carlo summary: each scenario is rerun many times over its random execution noise (missed sessions, session-to-session variation), and the plots show the median with a 10--90\% band. Figure~\ref{fig:regirr} contrasts regular execution of the Upper/Lower plan (every session) with an irregular schedule (about 55\% of sessions, at random). Regular execution settles high and steady with a tight band; the irregular schedule sits lower and its band is wide, because how much stimulus a haphazard week delivers depends heavily on which sessions happen to land.

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, ylabel={mean stimulus (\%)}, ymin=0, ymax=100, legend pos=south east]
\addplot[draw=none, forget plot, name path=rHi] table[x=week,y=regHi]{research/sim-ul.dat};
\addplot[draw=none, forget plot, name path=rLo] table[x=week,y=regLo]{research/sim-ul.dat};
\addplot[appBlue!15, forget plot] fill between[of=rHi and rLo];
\addplot[draw=none, forget plot, name path=iHi] table[x=week,y=irrHi]{research/sim-ul.dat};
\addplot[draw=none, forget plot, name path=iLo] table[x=week,y=irrLo]{research/sim-ul.dat};
\addplot[appAccent!15, forget plot] fill between[of=iHi and iLo];
\addplot[appBlue] table[x=week,y=regMid]{research/sim-ul.dat}; \addlegendentry{regular}
\addplot[appAccent] table[x=week,y=irrMid]{research/sim-ul.dat}; \addlegendentry{irregular}
\end{axis}\end{tikzpicture}
\caption{Regular (blue) versus irregular (orange) execution of the Upper/Lower plan: mean growth stimulus across nine muscles, median and 10--90\% band over 80 runs. Regular is tight and high; irregular is lower and wide.}\label{fig:regirr}
\end{figure}
...
\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, ylabel={mean stimulus (\%)}, ymin=0, ymax=100, legend pos=south east]
\addplot[draw=none, forget plot, name path=fbH] table[x=week,y=fbHi]{research/sim-plans.dat};
\addplot[draw=none, forget plot, name path=fbL] table[x=week,y=fbLo]{research/sim-plans.dat};
\addplot[appBlue!12, forget plot] fill between[of=fbH and fbL];
\addplot[draw=none, forget plot, name path=ulH] table[x=week,y=ulHi]{research/sim-plans.dat};
\addplot[draw=none, forget plot, name path=ulL] table[x=week,y=ulLo]{research/sim-plans.dat};
\addplot[appViolet!12, forget plot] fill between[of=ulH and ulL];
\addplot[draw=none, forget plot, name path=ppH] table[x=week,y=pplHi]{research/sim-plans.dat};
\addplot[draw=none, forget plot, name path=ppL] table[x=week,y=pplLo]{research/sim-plans.dat};
\addplot[appAccent!12, forget plot] fill between[of=ppH and ppL];
\addplot[appBlue] table[x=week,y=fbMid]{research/sim-plans.dat}; \addlegendentry{Full-Body 3x}
\addplot[appViolet] table[x=week,y=ulMid]{research/sim-plans.dat}; \addlegendentry{Upper/Lower 4x}
\addplot[appAccent] table[x=week,y=pplMid]{research/sim-plans.dat}; \addlegendentry{Push/Pull/Legs 6x}
\end{axis}\end{tikzpicture}
\caption{Mean growth stimulus across nine muscles for three standard plans, each executed regularly: median and 10--90\% band over 80 runs. More weekly volume per muscle lifts the whole-body stimulus, with diminishing returns.}\label{fig:plans}
\end{figure}
...
\subsection{Monte Carlo forecast}\label{sec:mc}
...
\Delta_t = b \cdot m \cdot \alpha \cdot \rho(D) \cdot \eta \cdot o \cdot e^{-t/32},
\end{equation}
where $\rho(D)$ is the dose--response of Section~4~\cite{sch17,drr}, $\eta$ scales with proximity to failure~\cite{failureprox}, $o$ is near 1 while progressing and drops without added load~\cite{plotkin}, $b$ is a training-age base rate~\cite{damas}, $\alpha$ is an age factor that attenuates hypertrophy past about 30~\cite{peterson}, and $m$ is the inter-individual response multiplier, drawn lognormal to span the range Hubal et~al.~\cite{hubal} observed (cross-sectional-area change from about $-2\%$ to $+59\%$). Sex enters as a separate lever but is set near neutral: relative hypertrophy is comparable between sexes~\cite{roberts}, so it informs absolute expectations rather than the percentage gain shown here. Below maintenance the increment turns negative (atrophy~\cite{mujika}), so a starved scenario trends down and the axis extends below zero. The first terms are well constrained by the literature; $b$ is literature-informed with a wide prior, and $m$ is the dominant source of spread, so the bands, not the median, carry the message. Figure~\ref{fig:mc} shows the median and 10--90\% range for the chosen plan against the current pace; the same scenarios drive the forecast in the app.

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[simaxis, xmax=16, ylabel={projected muscle gain (\%)}, ymin=0, legend pos=north west]
\addplot[draw=none, forget plot, name path=ppHi] table[x=week,y=planHi]{research/sim-mc.dat};
\addplot[draw=none, forget plot, name path=ppLo] table[x=week,y=planLo]{research/sim-mc.dat};
\addplot[appAccent!15, forget plot] fill between[of=ppHi and ppLo];
\addplot[draw=none, forget plot, name path=qqHi] table[x=week,y=paceHi]{research/sim-mc.dat};
\addplot[draw=none, forget plot, name path=qqLo] table[x=week,y=paceLo]{research/sim-mc.dat};
\addplot[appBlue!15, forget plot] fill between[of=qqHi and qqLo];
\addplot[appAccent, line width=1pt] table[x=week,y=planMid]{research/sim-mc.dat}; \addlegendentry{this plan}
\addplot[appBlue, line width=1pt] table[x=week,y=paceMid]{research/sim-mc.dat}; \addlegendentry{current pace}
\end{axis}\end{tikzpicture}
\caption{Monte Carlo muscle-gain forecast over 16 weeks (median lines, 10--90\% bands) for a novice: keeping to an adequate, progressing plan versus a lower, stalling current pace. The wide bands are chiefly inter-individual variability~\cite{hubal}.}\label{fig:mc}
\end{figure}

Beyond the individual-response spread, Figure~\ref{fig:sens} shows how the median 16-week gain moves when each input is swung across a plausible range (the app draws the same tornado). Training experience and progression move it most; volume and age move it least over the ranges tested. This is a forecast-level companion to the threshold sensitivity of Section~\ref{sec:verification}.

\begin{figure}[t]\centering
\begin{tikzpicture}\begin{axis}[xbar, width=\linewidth, height=4.6cm, xmin=0, xmax=6.2,
  ...
  xlabel={swing in 16-week median gain (percentage points)}, xlabel style={font=\footnotesize},
  tick label style={font=\footnotesize}, axis lines=left, grid=major, grid style={black!8},
  ytick={0,1,2,3,4}, yticklabels={effort, progression, volume, age, experience}]
\addplot[fill=appAccent!30, draw=appAccent] table[x=swing, y=y] {research/sim-sens.dat};
\end{axis}\end{tikzpicture}
\caption{Parameter sensitivity of the Monte Carlo forecast: how far the median 16-week gain swings as each input moves across a plausible range, around a novice plan baseline.}\label{fig:sens}
\end{figure}
