...
\Delta_t = b \cdot m \cdot \rho(D) \cdot \eta \cdot o \cdot e^{-t/32},
\end{equation}
where $\rho(D)$ is the dose--response of Section~4~\cite{sch17,drr}, $\eta$ scales with proximity to failure~\cite{failureprox}, $o$ is near 1 while progressing and drops without added load~\cite{plotkin}, $b$ is a training-age base rate~\cite{damas}, and $m$ is the inter-individual response multiplier, drawn lognormal to span the range Hubal et~al.~\cite{hubal} observed (cross-sectional-area change from about $-2\%$ to $+59\%$). The first four terms are well constrained by the literature; $b$ is literature-informed with a wide prior, and $m$ is the dominant source of spread, so the bands, not the median, carry the message. Figure~\ref{fig:mc} shows the median and 10--90\% range for the chosen plan against the current pace; the same two scenarios drive the forecast in the app.
