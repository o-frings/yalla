...
\end{equation}
where the sum runs over all entries $e$ logged in week $k$, and $n_e$, $f_e$, and $x_e$ are the entry's set count, effort code, and exercise. This same effort-weighted, fractional count feeds the weekly-sets chart, the balance radar, and the ``hard sets'' ring, so every surface in the app reads consistently.
...
Set strength is summarised by the Epley one-rep-max (1RM) estimate~\cite{epley}:
\begin{equation}\label{eq:epley}
\varphi(w,r)=w\left(1+\frac{r}{K}\right),
\end{equation}
where $w$ is the set's load (kg), $r$ its reps, and $K$ the rep denominator (population default $K=30$; Section~\ref{sec:kcal} calibrates it per lifter). The anchor for exercise $x$ is the best estimate over the lifter's \emph{prior} sessions for that exercise:
\begin{equation}\label{eq:anchor}
A_x=\max_{e\,\in\,\mathrm{hist}(x)}\varphi(w_e,r_e),
\end{equation}
where $\mathrm{hist}(x)$ is the set of the lifter's prior logged entries for exercise $x$ (the current, in-progress session is excluded) and $w_e$, $r_e$ are each entry's load and reps.
...
\end{equation}
where $A_x$ is the anchor \eqref{eq:anchor}, $w$ and $r$ are the load and reps of the set under evaluation, and $K$ is the rep denominator of \eqref{eq:epley}; the estimate is evaluated on the session's hardest set for that exercise (the one with the largest $\varphi$). It maps to an effort code:
...
\end{equation}
where $(w_1,r_1)$ and $(w_2,r_2)$ are the loads and reps of the two Max sets. Valid within-exercise pairs are pooled across the whole log, the median is taken and clamped to $[20,40]$, and $K=30$ is used until enough failure data exists. This grounds the effort estimate in the lifter's own data.
...
\end{equation}
where $S_g$ is the effective-set series \eqref{eq:S}, $L_g$ the load series just defined, and $\recent$ and $\earlier$ the window means of \eqref{eq:windows}. Each ratio is defined only when its earlier window is positive; otherwise it is treated as absent. The combined trend takes the better of the two:
...
\begin{itemize}[leftmargin=*]
\item \textbf{Validation status.} This is a mechanistic model, grounded in the literature and internally consistent. It has no validation against direct body-composition measurements (dual-energy X-ray absorptiometry, ultrasound, or magnetic resonance imaging): its thresholds are derived from published dose--response findings, none are fitted to an outcome dataset, and it claims no predictive-accuracy interval. The checks in Section~\ref{sec:verification} cover internal consistency and coherence; external accuracy stays out of scope.
