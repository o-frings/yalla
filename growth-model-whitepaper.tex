...
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}\centering
\begin{tikzpicture}\begin{axis}[xbar, width=\linewidth, height=4.7cm, xmin=0, xmax=7, bar width=7pt, ymin=-0.9, ymax=4.9, clip=false, nodes near coords, point meta=x, nodes near coords style={font=\scriptsize, /pgf/number format/fixed, /pgf/number format/precision=1}, xlabel={16-wk gain swing (pp)}, xlabel style={font=\footnotesize}, title={\footnotesize Parameter sensitivity}, tick label style={font=\footnotesize}, axis lines=left, grid=major, grid style={black!8}, ytick={0,1,2,3,4}, yticklabels={age, volume, effort, progression, experience}]
\addplot[fill=appAccent!30, draw=appAccent] table[x=swing, y=y] {research/sim-sens.dat};
\end{axis}\end{tikzpicture}
\end{minipage}
\caption{\emph{Left:} the same progressing plan from three starting points (training-age base rates); newer lifters gain faster, all bands wide and overlapping. \emph{Right:} swing in the 16-week median gain as each input moves across a plausible range, sorted by influence.}\label{fig:starts}
\end{figure}
