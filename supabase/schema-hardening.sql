...
-- 10. friend suggestions ("people you may know") — see schema-suggestions.sql --
...
language sql stable security definer set search_path = '' as $$
  ...
  myf as (select f.followee as uid from public.follows f, me
          where f.follower = me.id and f.status = 'accepted'),
  excluded as (select f.followee as uid from public.follows f, me where f.follower = me.id
               union select id from me),
  fof as (select f.followee as uid, count(distinct f.follower) as mutuals
          from public.follows f
          where f.follower in (select uid from myf) and f.status = 'accepted'
          group by f.followee),
  followers as (select f.follower as uid from public.follows f, me
                where f.followee = me.id and f.status = 'accepted'),
  pub as (select p.user_id as uid from public.profiles p where p.visibility = 'public'),
  cand as (select uid, mutuals, false as fy from fof
           union all select uid, 0, true  from followers
           union all select uid, 0, false from pub),
  agg as (select uid, max(mutuals) as mutuals, bool_or(fy) as follows_you
          from cand group by uid)
  select a.uid, p.display_name, p.visibility, a.mutuals::int, a.follows_you
  from agg a join public.profiles p on p.user_id = a.uid
