...
returns table(user_id uuid, display_name text, avatar_color text, avatar_emoji text, avatar_style text, last_seen timestamptz, created_at timestamptz)
language sql stable security definer set search_path = '' as $$
  select p.user_id, p.display_name, p.avatar_color, p.avatar_emoji, p.avatar_style, p.last_seen, f.created_at
...
returns table(user_id uuid, display_name text, avatar_color text, avatar_emoji text, avatar_style text, last_seen timestamptz, status text)
language sql stable security definer set search_path = '' as $$
  select p.user_id, p.display_name, p.avatar_color, p.avatar_emoji, p.avatar_style, p.last_seen, f.status
...
returns table(user_id uuid, display_name text, avatar_color text, avatar_emoji text, avatar_style text, last_seen timestamptz, live boolean)
language sql stable security definer set search_path = '' as $$
  select p.user_id, p.display_name, p.avatar_color, p.avatar_emoji, p.avatar_style, p.last_seen, f.live
  from public.follows f join public.profiles p on p.user_id = f.follower
...
returns table(user_id uuid, display_name text, avatar_color text, avatar_emoji text, avatar_style text, last_seen timestamptz, visibility text, mutuals int, follows_you boolean)
language sql stable security definer set search_path = '' as $$
  ...
  select a.uid, p.display_name, p.avatar_color, p.avatar_emoji, p.avatar_style, p.last_seen, p.visibility, a.mutuals::int, a.follows_you
  from agg a
